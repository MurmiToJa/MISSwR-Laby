...
%% Planowanie ścieżki przy użyciu mobileRobotPRM
planPathWithMobileRobotPRM(mapOccupancy, start_point, end_point, numNodesList);
...
% Funkcja planowania ścieżki przy użyciu mobileRobotPRM
function planPathWithMobileRobotPRM(map, start_point, end_point, numNodesList)
    for idx = 1:length(numNodesList)
        nodes = numNodesList(idx);
        prmSimple = mobileRobotPRM(map, nodes);
        
        figure;
        path = findpath(prmSimple, start_point, end_point);
        show(prmSimple)
        title(sprintf("mobileRobotPRM z nodes = %d", nodes));
    end
end